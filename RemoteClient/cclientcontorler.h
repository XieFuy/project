...
public:
    ...
    CPacket downLoadFileFromRemote(std::string& data,char* packetBuffer = nullptr,size_t* nSize = nullptr); //本地主机从远程主机下载文件
    CPacket upDataFileToRemote(std::string& data,char* packetBuffer = nullptr,size_t* nSize = nullptr);//本地主机向远程主机传输文件
private:
